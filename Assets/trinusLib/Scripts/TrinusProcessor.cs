		...
		void Awake () {
			...
			if (trinusManager == null) {
				...
				if (!trinusManager.isIdValid())
					Debug.LogWarning ("DEV MODE ON: No Valid Project Id set in TrinusManager. Email info@trinusvr.com to obtain one.");
			} else {
				return;
			}
...
//				else
			...
			case DataStructs.STATUS.CONNECTING:
				setMessage ("msgTrinusWait", (string.IsNullOrEmpty(trinusManager.getLastResult ().detail)? "" : "STEP " + trinusManager.getLastResult ().detail));
...
//				if (resetViewFlag) {
//					Vector3 defaultRotation = trinusCamera.getDefaultRotation ();
//					//Debug.Log ("Default rotation : " + defaultRotation);
//					if (resetYawOnly)
//						offset = new Vector3 (sensorData.yaw + defaultRotation.y, 90, 0);
//					else
//						offset = new Vector3 (sensorData.yaw + defaultRotation.y,
//							-defaultRotation.x + 90, sensorData.roll + defaultRotation.z);
//					resetViewFlag = false;
//					offset = new Vector3 (sensorData.yaw, 0, 0);
//					test.Set(sensorData.quatX, sensorData.quatY, sensorData.quatZ, sensorData.quatW);
//				}
//				float yaw = (sensorData.yaw - offset.x) * settingsInstance.yawScale;
//				float pitch = sensorData.pitch - offset.y;
//				float roll = -sensorData.roll + offset.z;
//				if (!ignoreTracking && trinusCamera != null)
//					trinusCamera.transform.localEulerAngles = new Vector3 (pitch, yaw, roll);
				acceleration.Set (sensorData.accelX, sensorData.accelY, sensorData.accelZ);
				//TODO do yaw turn ratio for quaternion
				if (settingsInstance.yawScale != 1) {
					if (!ignoreTracking && trinusCamera != null)
					...
					//Quaternion rotation = new Quaternion (sensorData.quatX, sensorData.quatY, sensorData.quatZ, sensorData.quatW);// * Quaternion.AngleAxis (offset.x, Vector3.up);
					//cameraRotation.Set(sensorData.quatX, sensorData.quatY, sensorData.quatZ, sensorData.quatW);
...
//					sensorDataDelegate (rotation);
//					sensorDataDelegate (yaw, pitch, roll);
				}
				break;
			}
		}
	...
	//	float delta = 1.5f;
	//	float smooth(float c, float n, float t){
	//		if (Math.Abs (c - n) < t) {
	//			Debug.Log ("Lerping");
	//			return Mathf.LerpAngle (c, n, Time.deltaTime);
	//		}
	//		return n;
	//	}
		/// <summary>
		...
		public void resetTracking(){
			//resetViewFlag = true;
//			resetView (defaultYaw);
			if (trinusManager != null)
				trinusManager.resetSensor ();
		}
//		public void resetView(float startingYaw){
//			resetViewFlag = true;
////			if (trinusManager != null && trinusManager.getStatus () == DataStructs.STATUS.STREAMING) {
////				DataStructs.SENSORS sensorData = trinusManager.getSensorData();
////				offset = new Vector3(sensorData.yaw, sensorData.pitch, sensorData.roll);
////			}
//		}
...
//						RenderTexture.active = tex;
					...
					{
						transferThread = new Thread(textureToBitmap);
						transferThread.Priority = System.Threading.ThreadPriority.Highest;
		...
		System.Drawing.Bitmap bitmap = null;
		System.Drawing.Rectangle bitsRect;
		AutoResetEvent transferReady = new AutoResetEvent(false);
		void textureToBitmap(){
			while (trinusManager.getStatus() == DataStructs.STATUS.STREAMING) {
				transferReady.WaitOne();
				lock(this)
				{
					bmpData = bitmap.LockBits (bitsRect, System.Drawing.Imaging.ImageLockMode.WriteOnly, System.Drawing.Imaging.PixelFormat.Format24bppRgb);
					System.Runtime.InteropServices.Marshal.Copy (bytes, 0, bmpData.Scan0, bytes.Length);
					bitmap.UnlockBits (bmpData);
					trinusManager.sendNextFrame (bitmap, settingsInstance.compressionQuality);
					//Debug.Log (Manager.tmpLog);
				}
			}
			transferThread = null;
		}
